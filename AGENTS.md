...
- 2026-09-06: a symlink guard added to a path-traversal fix covered the reference path it was written for, but not a sibling entry point (a directory-discovery glob, which matches a symlinked directory same as a real one) — found only by a follow-up review after a purpose-built branch-wide security pass had already said "no issues found." Rule: symlink/path-traversal validation is an enumeration problem, not a single fix — list every place an untrusted name enters a path (every glob, every parsed reference) before considering the guard complete.
