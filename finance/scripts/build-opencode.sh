...
# Generates .opencode/commands/finance-*.md and .opencode/agents/*.md from the
# canonical Claude Code skills and agent definitions in finance/.
# Run from anywhere: ./finance/scripts/build-opencode.sh
#
...
build_command() {
  ...
  output_name="finance-$skill_name"
...

  {
    ...
    # OpenCode does not substitute ${CLAUDE_PLUGIN_ROOT}, and its users run the
    # build from a checkout, so the plugin directory is known here. Replace the
    # literal with the absolute path so toolkit script paths work as written.
    # The braces are left unescaped on the pattern side because BSD sed reads
    # \{ \} as an interval expression; a bare { } is literal in a BRE.
    local escaped_root
    escaped_root=$(printf '%s' "$PLUGIN_DIR" | sed 's/[\/&\\]/\\&/g')
    body=$(echo "$body" | sed 's/\${CLAUDE_PLUGIN_ROOT}/'"$escaped_root"'/g')
...
# Helper: convert a plugin agent definition (finance/agents/*.md) to an
...
if [[ "$built" == 0 ]]; then
  echo "Nothing to build: finance/ has no skills or agents yet."
fi
